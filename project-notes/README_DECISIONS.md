...
長文化したRoot `README.md` の冒頭へ `Contents（目次）` を追加する。

目次には、主要章1〜9と、Personaの効果を示す主要な補助節 `3.1 Personaあり／なしの検証例（Code Generator）` だけを掲載する。各項目は固定アンカーを使用して該当箇所へ移動できるようにする。

Reason:

Root READMEはRepositoryの入口であり、比較表と評価項目説明の追加によって全文を上から読み進めないと目的箇所へ移動しにくくなったため。

一方で、すべての小見出しまで目次へ含めると目次自体が長くなり、入口文書の可読性を逆に損なうため、主要章と重要な比較節だけに限定する。

Rejected:

- 目次を置かず、全文スクロールだけで移動する方式
- 評価項目などすべての小見出しを列挙する詳細目次
- README本文を複数ファイルへ分割して入口情報を過度に分散する方式
