...
- Learning Moduleの詳細検索範囲は、ユーザーが保有する職業訓練カリキュラムPDFを基準に判断する。
- 一般論から検索範囲を先に確定しない。
- 対象PDFの内容を確認した後、その内容に基づいてLearning Moduleの具体的な検索対象・検索範囲を定義する。
...
- Learning Moduleの詳細検索対象・検索範囲（職業訓練カリキュラムPDF確認後に確定）
