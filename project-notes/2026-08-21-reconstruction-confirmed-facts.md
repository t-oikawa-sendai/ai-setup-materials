...
- Researcher Personaは、Gemへ設定する時点では必要な本体・Moduleを1本のファイルとして読み込ませる方向とする。
...
- 教材としてあらかじめ提供する対象は、現時点では次の3段階を想定する。
  1. `Learning + Career + Development`
  2. `Learning + Development`
  3. `Development`
- 上記3段階をどのファイル構成・入替方式で提供するかの最終確定は、別途行う。
...
- 上記3段階を3つの完成済みResearcherファイルとして提供する方式を正式採用するか
- 3分野それぞれの詳細な検索対象・検索範囲
- Researcher Persona本文内でのModule配置・記法
- 3パターン間でResearcher本体をどう一元管理し、内容不一致を防止するか
