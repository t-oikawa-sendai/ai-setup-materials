...
### SUBSEQUENTLY USER-CONFIRMED: 2026-08-22

上記再利用候補のうち、役割名・戻し先・実施担当に依存しないReviewerの中核部分は、現行Reviewerでも維持することをユーザー確認済みとする。
...
- 品質・保守性・セキュリティ
- 秘密情報・個人情報の不適切な露出
- Evidenceのない指摘を確定事項として扱わないこと
- 好みだけで変更を要求しないこと
- 問題がない範囲へ不要な改善提案を広げないこと
- 対象に関係する観点だけを評価し、全観点を機械的に列挙しないこと

旧Reviewerの `Implementer` 依存部分と旧出力形式はこの確認に含めず、後続のUser-first仕様へ置き換える。
...
27. Reviewerは、詳細レビューを始める前に、レビューに必要な対象・資料・検証Evidenceがそろっているか受付確認を行う。
28. 必要なものが不足しレビューを行えない場合は、総合判定を `BLOCKED`、状態を「レビュー不能」と明記する。
29. レビュー不能時は、初心者であるUserが次に行う作業をそのまま実行できるよう、レビューを行えない理由、不足しているもの、Userが行う作業、準備するもの、具体的なEvidence例、提出方法、再レビューを開始できる条件を示す。
30. 初心者向けの案内では、Evidenceを必要に応じて「確認に必要な記録・資料」と説明する。
31. Evidence例は全案件共通の固定リストとして機械的に要求せず、その案件で不足しているものに対応する具体例だけを示す。
